...
function hotspotMarkerHtml(h,i,mode='editor'){
 ...
 if(type==='number')return `<span class="${cls}" style="--hot-size:${size}px">${i+1}</span>`;
 if(type==='image'){
  const src=h.markerAssetId?`<img data-learn-local-asset="${esc(h.markerAssetId)}" alt="">`:h.markerImage?`<img src="${esc(h.markerImage)}" alt="">`:'🖼️';
  return `<span class="${cls}" style="--hot-size:${size}px">${src}</span>`;
 }
 if(type==='text')return `<span class="${cls}" style="--hot-size:${size}px">${esc(value||h.label||'Abrir')}</span>`;
 return `<span class="${cls}" style="--hot-size:${size}px">${esc(value||'🔎')}</span>`;
}
...
function elementMarkup(e,p){
 ...
 if(e.type==='hotspot'){const img=e.assetId?`<img data-learn-local-asset="${esc(e.assetId)}" alt="${esc(e.alt||'Imagem interativa')}">`:e.src?`<img src="${esc(e.src)}" alt="${esc(e.alt||'Imagem interativa')}">`:'<div class="image-placeholder">✨ Adicione a imagem do cenário</div>';return `<div class="hotspot-editor-wrap"><div class="hotspot-preview hotspot-click-surface" data-hotspot-editor="${esc(e.id)}">${img}${(e.hotspots||[]).map((h,i)=>`<button type="button" class="hotspot-editor-marker" data-hot-editor-id="${esc(h.id)}" style="left:${Number(h.x)||50}%;top:${Number(h.y)||50}%" aria-label="${esc(h.label||'Ponto '+(i+1))}">${hotspotMarkerHtml(h,i,'editor')}</button>`).join('')}</div><small class="hotspot-click-hint">✨ Clique em qualquer ponto da imagem para adicionar uma interação.</small></div>`;}
 ...
 if(e.type==='socialcarousel'){const s=e.slides?.[0]||{};return `<div class="social-carousel-preview"><div class="social-phone-mini"><div class="social-mini-top"><div class="social-avatar-mini">${esc((e.accountName||'P').slice(0,1))}</div><div><b>${esc(e.accountName||'Perfil pedagógico')}</b><small>${esc(e.accountSubtitle||'')}</small></div><span>•••</span></div><div class="social-mini-hero">${s.imageUrl?`<img src="${esc(s.imageUrl)}" alt="">`:'<div class="social-mini-placeholder"></div>'}<span class="social-mini-count">1/${e.slides?.length||1}</span><div><small>${esc(s.kicker||'')}</small><h3>${esc(s.title||'Carrossel social')}</h3><p>${esc(s.subtitle||'')}</p></div></div><div class="social-mini-actions">♡　◯　⌁　　♧</div><div class="social-mini-dots">${(e.slides||[]).map((_,i)=>`<i class="${i===0?'active':''}"></i>`).join('')}</div></div><div><b>Carrossel Social</b><p>${e.slides?.length||0} slide(s)</p></div></div>`;
...
function toolbox(){
 return `<div class="learn-toolbox advanced">
  ...
  <section class="tool-group"><h4>📱 Simuladores pedagógicos</h4><div>
   ...
   <button data-add-block="socialcarousel"><span>▣</span><b>Carrossel Social</b><small>Celular + swipe + perfil + slides</small></button>
  </div></section>
...
function properties(e,p){
 ...
 if(e.type==='hotspot')return `<div class="learn-props"><input id="propHotspotImageFile" type="file" accept="image/*" hidden><button class="soft" id="propChooseHotspotImage" type="button">📁 Escolher imagem/cenário</button><label>Ou URL da imagem<input id="propHotspotSrc" value="${esc(e.src||'')}"></label><label>Texto alternativo da imagem<input id="propHotspotAlt" value="${esc(e.alt||'')}"></label><div class="hotspot-builder-tip">✨ <b>Adicionar pontos:</b> clique diretamente no lugar desejado da imagem. Os campos X/Y continuam disponíveis só para ajuste fino.</div><div class="checkpoint-head"><b>Pontos interativos</b><button class="tiny" id="propAddHotspot" type="button">＋ Ponto central</button></div>${(e.hotspots||[]).map((h,i)=>`<section class="mini-prop-card hotspot-prop-card" data-hot-id="${h.id}"><div class="checkpoint-item-head"><b>Ponto ${i+1} · ${esc(h.label||'Explorar')}</b><button class="tiny danger-text" data-hot-delete="${h.id}" type="button">Excluir</button></div><label>Rótulo acessível<input data-hot-field="label" data-hot-id="${h.id}" value="${esc(h.label||'Explorar')}"></label><div class="hot-marker-grid"><label>Aparência<select data-hot-field="markerType" data-hot-id="${h.id}"><option value="number" ${h.markerType==='number'?'selected':''}>Número</option><option value="emoji" ${!h.markerType||h.markerType==='emoji'?'selected':''}>Emoji / símbolo</option><option value="text" ${h.markerType==='text'?'selected':''}>Palavra / texto</option><option value="image" ${h.markerType==='image'?'selected':''}>Imagem própria</option></select></label><label>Conteúdo do marcador<input data-hot-field="markerValue" data-hot-id="${h.id}" value="${esc(h.markerValue||'🔎')}" placeholder="🔎, 📖, 📰, ➜, Saiba mais..."></label></div><div class="hot-marker-presets"><button type="button" data-hot-preset="🔎" data-hot-id="${h.id}">🔎 Lupa</button><button type="button" data-hot-preset="📖" data-hot-id="${h.id}">📖 Livro</button><button type="button" data-hot-preset="📰" data-hot-id="${h.id}">📰 Artigo</button><button type="button" data-hot-preset="➜" data-hot-id="${h.id}">➜ Caminho</button><button type="button" data-hot-preset="📍" data-hot-id="${h.id}">📍 Pin</button><button type="button" data-hot-preset="✨" data-hot-id="${h.id}">✨ Destaque</button><button type="button" data-hot-preset="ℹ️" data-hot-id="${h.id}">ℹ️ Info</button></div><input type="file" accept="image/*" data-hot-marker-file="${h.id}" hidden><button type="button" class="soft" data-hot-marker-upload="${h.id}">🖼️ Usar imagem como marcador</button><label>Ou URL da imagem do marcador<input data-hot-field="markerImage" data-hot-id="${h.id}" value="${esc(h.markerImage||'')}"></label><div class="hot-marker-grid"><label>Estilo<select data-hot-field="markerStyle" data-hot-id="${h.id}"><option value="glow" ${!h.markerStyle||h.markerStyle==='glow'?'selected':''}>Brilhante</option><option value="pulse" ${h.markerStyle==='pulse'?'selected':''}>Pulsante</option><option value="solid" ${h.markerStyle==='solid'?'selected':''}>Sólido</option><option value="minimal" ${h.markerStyle==='minimal'?'selected':''}>Discreto</option><option value="label" ${h.markerStyle==='label'?'selected':''}>Etiqueta</option></select></label><label>Cor<input type="color" data-hot-field="markerColor" data-hot-id="${h.id}" value="${esc(h.markerColor||'#7d6bd6')}"></label></div><label>Tamanho <span class="field-help">${Number(h.markerSize)||40}px</span><input type="range" min="22" max="88" data-hot-field="markerSize" data-hot-id="${h.id}" value="${Number(h.markerSize)||40}"></label><div class="support-time-grid"><label>X (%)<input data-hot-field="x" data-hot-id="${h.id}" type="number" min="0" max="100" step=".1" value="${Number(h.x)||50}"></label><label>Y (%)<input data-hot-field="y" data-hot-id="${h.id}" type="number" min="0" max="100" step=".1" value="${Number(h.y)||50}"></label></div><label>Ao clicar<select data-hot-field="action" data-hot-id="${h.id}"><option value="popup" ${h.action==='popup'?'selected':''}>Abrir caixa / conteúdo</option><option value="scene" ${h.action==='scene'?'selected':''}>Ir para outra tela/cena</option><option value="effect" ${h.action==='effect'?'selected':''}>Disparar efeito</option><option value="link" ${h.action==='link'?'selected':''}>Abrir artigo/link/material</option></select></label><label>Conteúdo da caixa<textarea data-hot-field="content" data-hot-id="${h.id}">${esc(h.content||'')}</textarea></label><label>Tela/cena de destino<select data-hot-field="targetSlideId" data-hot-id="${h.id}"><option value="">Nenhuma</option>${p.slides.map(s=>`<option value="${s.id}" ${h.targetSlideId===s.id?'selected':''}>${esc(s.title)}</option>`).join('')}</select></label><label>Efeito<select data-hot-field="effect" data-hot-id="${h.id}">${['confetti','applause','gift','stars','balloons','boo'].map(x=>`<option value="${x}" ${h.effect===x?'selected':''}>${x}</option>`).join('')}</select></label><label>Link/artigo/material<input data-hot-field="url" data-hot-id="${h.id}" value="${esc(h.url||'')}" placeholder="https://..."></label></section>`).join('')}</div>`;
 ...
 if(e.type==='socialcarousel')return `<div class="learn-props"><input id="propSocialAvatarFile" type="file" accept="image/*" hidden><button class="soft" id="propSocialAvatarChoose" type="button">📷 Foto de perfil</button><label>Nome do perfil<input id="propSocialAccount" value="${esc(e.accountName||'')}"></label><label>Subtítulo<input id="propSocialSubtitle" value="${esc(e.accountSubtitle||'')}"></label><label>Legenda da publicação<textarea id="propSocialCaption">${esc(e.caption||'')}</textarea></label><label>Linha de curtidas<input id="propSocialLikes" value="${esc(e.likes||'')}"></label><label>Chamada de comentários<input id="propSocialComments" value="${esc(e.comments||'')}"></label><label>Rodapé/tempo<input id="propSocialTime" value="${esc(e.time||'')}"></label><div class="checkpoint-head"><b>Slides</b><button class="tiny" id="propSocialAddSlide" type="button">＋ Slide</button></div>${(e.slides||[]).map((s,i)=>`<section class="mini-prop-card social-slide-props" data-social-slide="${s.id}"><div class="checkpoint-item-head"><b>Slide ${i+1}</b><button class="tiny danger-text" type="button" data-social-remove="${s.id}" ${(e.slides||[]).length<=1?'disabled':''}>Remover</button></div><input type="file" accept="image/*" data-social-file="${s.id}" hidden><button class="soft" type="button" data-social-choose="${s.id}">🖼️ Imagem do slide</button><label>Ou URL da imagem<input data-social-field="imageUrl" data-social-id="${s.id}" value="${esc(s.imageUrl||'')}"></label><label>Ajuste<select data-social-field="fit" data-social-id="${s.id}"><option value="cover" ${s.fit!=='contain'?'selected':''}>Preencher</option><option value="contain" ${s.fit==='contain'?'selected':''}>Mostrar inteira</option></select></label><label>Rótulo superior<input data-social-field="topLabel" data-social-id="${s.id}" value="${esc(s.topLabel||'')}"></label><label>Faixa pequena<input data-social-field="kicker" data-social-id="${s.id}" value="${esc(s.kicker||'')}"></label><label>Título<textarea data-social-field="title" data-social-id="${s.id}">${esc(s.title||'')}</textarea></label><label>Subtítulo<textarea data-social-field="subtitle" data-social-id="${s.id}">${esc(s.subtitle||'')}</textarea></label><label>Chip<input data-social-field="chip" data-social-id="${s.id}" value="${esc(s.chip||'')}"></label><label>Tipo de conteúdo<select data-social-field="bodyType" data-social-id="${s.id}"><option value="text" ${s.bodyType==='text'?'selected':''}>Texto</option><option value="grid" ${s.bodyType==='grid'?'selected':''}>4 destaques</option><option value="quote" ${s.bodyType==='quote'?'selected':''}>Recorte/citação</option><option value="cta" ${s.bodyType==='cta'?'selected':''}>Chamada final</option></select></label>${s.bodyType==='grid'?`<label>Destaques <span class="field-help">Título :: texto, um por linha</span><textarea data-social-field="gridText" data-social-id="${s.id}">${esc((s.items||[]).map(x=>(x.title||'')+' :: '+(x.text||'')).join('\n'))}</textarea></label>`:s.bodyType==='quote'?`<label>Recorte/citação<textarea data-social-field="quote" data-social-id="${s.id}">${esc(s.quote||'')}</textarea></label><label>Complemento<textarea data-social-field="bodyText" data-social-id="${s.id}">${esc(s.bodyText||'')}</textarea></label>`:s.bodyType==='cta'?`<label>Título da chamada<input data-social-field="ctaTitle" data-social-id="${s.id}" value="${esc(s.ctaTitle||'')}"></label><label>Texto da chamada<textarea data-social-field="ctaText" data-social-id="${s.id}">${esc(s.ctaText||'')}</textarea></label>`:`<label>Título inferior<input data-social-field="bodyTitle" data-social-id="${s.id}" value="${esc(s.bodyTitle||'')}"></label><label>Texto inferior<textarea data-social-field="bodyText" data-social-id="${s.id}">${esc(s.bodyText||'')}</textarea></label>`}</section>`).join('')}</div>`;
...
function newElement(type){
 ...
 if(type==='hotspot')return{id:uid('el'),type,src:'',assetId:'',alt:'Imagem interativa',hotspots:[{id:uid('hot'),x:28,y:35,label:'Explorar',markerType:'emoji',markerValue:'🔎',markerStyle:'glow',markerSize:40,markerColor:'#7d6bd6',markerImage:'',markerAssetId:'',action:'popup',content:'Conteúdo deste ponto.',targetSlideId:'',effect:'confetti',url:''}]};
 ...
 if(type==='socialcarousel')return{id:uid('el'),type,accountName:'Perfil pedagógico',accountSubtitle:'Pesquisa • Aprendizagem • Carreira',caption:'Leia o carrossel e participe da discussão.',likes:'Curtido por estudantes da turma',comments:'Participe da discussão',time:'material complementar',avatarAssetId:'',avatarUrl:'',slides:[
  {id:uid('socialslide'),imageAssetId:'',imageUrl:'',fit:'cover',topLabel:'CONTEÚDO\nEM DESTAQUE',kicker:'LEITURA VISUAL',title:'Título do primeiro slide',subtitle:'Use este espaço para introduzir a temática.',chip:'APRENDIZAGEM • CARROSSEL',bodyType:'text',bodyTitle:'Panorama inicial',bodyText:'Escreva aqui o conteúdo complementar do slide.'},
  {id:uid('socialslide'),imageAssetId:'',imageUrl:'',fit:'cover',topLabel:'DESTAQUE\nDA AULA',kicker:'PONTO IMPORTANTE',title:'Organize a informação em sequência',subtitle:'O aluno desliza os slides como em um carrossel social.',chip:'',bodyType:'grid',items:[{title:'Ideia 1',text:'Explique o primeiro ponto.'},{title:'Ideia 2',text:'Explique o segundo ponto.'},{title:'Ideia 3',text:'Explique o terceiro ponto.'},{title:'Ideia 4',text:'Explique o quarto ponto.'}]},
  {id:uid('socialslide'),imageAssetId:'',imageUrl:'',fit:'cover',topLabel:'PARA\nREFLETIR',kicker:'INTERAÇÃO',title:'Finalize com uma pergunta ou chamada',subtitle:'Conecte o carrossel ao fórum, atividade ou discussão.',chip:'',bodyType:'cta',ctaTitle:'💬 Sua vez',ctaText:'Compartilhe sua percepção sobre o tema.'}
 ]};
...
function bindProperties(){
 ...
 if(e.type==='hotspot'){
  ...
  const addHot=(x=50,y=50)=>{e.hotspots??=[];e.hotspots.push({id:uid('hot'),x:Number(x.toFixed(1)),y:Number(y.toFixed(1)),label:'Explorar',markerType:'emoji',markerValue:'🔎',markerStyle:'glow',markerSize:40,markerColor:'#7d6bd6',markerImage:'',markerAssetId:'',action:'popup',content:'Conteúdo deste ponto.',targetSlideId:'',effect:'confetti',url:''});touch();render()};
  $('#propAddHotspot',root)?.addEventListener('click',()=>addHot());
  const surface=$('[data-hotspot-editor]',root);if(surface)surface.onclick=ev=>{if(ev.target.closest('[data-hot-editor-id]'))return;const rect=surface.getBoundingClientRect(),x=Math.max(0,Math.min(100,((ev.clientX-rect.left)/rect.width)*100)),y=Math.max(0,Math.min(100,((ev.clientY-rect.top)/rect.height)*100));addHot(x,y)};
  $$('[data-hot-editor-id]',root).forEach(btn=>btn.onclick=ev=>{ev.stopPropagation();const card=$(`[data-hot-id="${btn.dataset.hotEditorId}"]`,root);if(card){card.classList.add('hot-card-focus');card.scrollIntoView({behavior:'smooth',block:'nearest'});setTimeout(()=>card.classList.remove('hot-card-focus'),1400)}});
  $$('[data-hot-delete]',root).forEach(n=>n.onclick=()=>{e.hotspots=e.hotspots.filter(h=>h.id!==n.dataset.hotDelete);touch();render()});
  $$('[data-hot-field]',root).forEach(n=>{const apply=()=>{const h=e.hotspots.find(x=>x.id===n.dataset.hotId);if(!h)return;const key=n.dataset.hotField;h[key]=['x','y','markerSize'].includes(key)?Number(n.value):n.value;touch();renderCanvasLight();hydrateLocalVideos(root).catch(()=>{})};n.oninput=apply;n.onchange=apply});
  $$('[data-hot-preset]',root).forEach(n=>n.onclick=()=>{const h=e.hotspots.find(x=>x.id===n.dataset.hotId);if(!h)return;h.markerType='emoji';h.markerValue=n.dataset.hotPreset;touch();render()});
  $$('[data-hot-marker-upload]',root).forEach(n=>n.onclick=()=>root.querySelector(`[data-hot-marker-file="${n.dataset.hotMarkerUpload}"]`)?.click());
  $$('[data-hot-marker-file]',root).forEach(n=>n.onchange=async()=>{const h=e.hotspots.find(x=>x.id===n.dataset.hotMarkerFile),picked=n.files?.[0];if(!h||!picked)return;const assetId=uid('hotmark');await putLearnMedia(assetId,picked);h.markerAssetId=assetId;h.markerType='image';touch();render();toast('Imagem usada como marcador.')});
 }
 ...
 if(e.type==='socialcarousel'){
  bind('#propSocialAccount','accountName');bind('#propSocialSubtitle','accountSubtitle');bind('#propSocialCaption','caption');bind('#propSocialLikes','likes');bind('#propSocialComments','comments');bind('#propSocialTime','time');
  const avatarChoose=$('#propSocialAvatarChoose',root),avatarFile=$('#propSocialAvatarFile',root);if(avatarChoose&&avatarFile){avatarChoose.onclick=()=>avatarFile.click();avatarFile.onchange=async()=>{const picked=avatarFile.files?.[0];if(!picked)return;const assetId=uid('socialavatar');await putLearnMedia(assetId,picked);e.avatarAssetId=assetId;touch();render();toast('Foto de perfil adicionada.')}};
  $('#propSocialAddSlide',root)?.addEventListener('click',()=>{e.slides.push({id:uid('socialslide'),imageAssetId:'',imageUrl:'',fit:'cover',topLabel:'NOVO\nSLIDE',kicker:'NOVO RECORTE',title:'Novo título',subtitle:'Adicione uma introdução curta.',chip:'',bodyType:'text',bodyTitle:'Informação',bodyText:'Escreva o conteúdo deste slide.'});touch();render()});
  $$('[data-social-remove]',root).forEach(b=>b.onclick=()=>{if(e.slides.length<=1)return;e.slides=e.slides.filter(x=>x.id!==b.dataset.socialRemove);touch();render()});
  $$('[data-social-choose]',root).forEach(b=>b.onclick=()=>root.querySelector(`[data-social-file="${b.dataset.socialChoose}"]`)?.click());
  $$('[data-social-file]',root).forEach(n=>n.onchange=async()=>{const s=e.slides.find(x=>x.id===n.dataset.socialFile),picked=n.files?.[0];if(!s||!picked)return;const assetId=uid('socialimg');await putLearnMedia(assetId,picked);s.imageAssetId=assetId;touch();render();toast('Imagem do slide adicionada.')});
  $$('[data-social-field]',root).forEach(n=>{const apply=()=>{const s=e.slides.find(x=>x.id===n.dataset.socialId);if(!s)return;const key=n.dataset.socialField;if(key==='gridText'){s.items=splitLines(n.value).map(line=>{const [title,...rest]=line.split('::');return{title:(title||'').trim(),text:rest.join('::').trim()}})}else{s[key]=n.value}touch();if(key==='bodyType')render();else renderCanvasLight()};n.oninput=apply;n.onchange=apply});
 }
...
function exportedHtml(p){
 ...
 function advElem(e){
  ...
  if(e.type==='hotspot')return'<div class="hotspot" data-el-id="'+esc(e.id)+'"><div class="hot-image">'+imgMarkup(e)+((e.hotspots||[]).map((h,i)=>{const type=h.markerType||'emoji',value=h.markerValue||'🔎',size=Math.max(22,Math.min(88,Number(h.markerSize)||40)),style=h.markerStyle||'glow',content=type==='number'?String(i+1):type==='image'?(h.markerImage?'<img src="'+esc(h.markerImage)+'" alt="">':'🖼️'):esc(value||h.label||'Abrir');return'<button type="button" class="hot-dot marker-'+esc(style)+' type-'+esc(type)+'" data-hot="'+i+'" style="left:'+(Number(h.x)||50)+'%;top:'+(Number(h.y)||50)+'%;--hot-size:'+size+'px;--hot-color:'+esc(h.markerColor||'#7d6bd6')+'" aria-label="'+esc(h.label||'Explorar')+'">'+content+'</button>'}).join(''))+'</div><div class="hot-panel" hidden></div></div>';
